...
/**
 * The resolution ladder behind the search box, extracted from `/api/resolve` so the no-JS
 * fallback route (`/search`) answers exactly the way the JSON route does. Two copies of "what
 * does this query mean" would drift into two different search engines; this is the only one.
 */
